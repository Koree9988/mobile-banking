  ...
  <q-page>
    <q-card square flat class="blue-color-top">
      <q-card-section class="row q-my-auto absolute-bottom-left">
        <div class="col">
          <q-btn v-if="nextProcess" flat class="text-white" @click="backProcess">
            <q-icon
              round
              size="2rem"
              name="arrow_back_ios"
          ...
          <q-btn v-if="!nextProcess" flat class="text-white" @click="toRoute('/menu/home')">
            <q-icon
              round
              size="2rem"
              name="arrow_back_ios"
    ...
    <q-card class="opacity-80" style="height: 78vh">
      ...
      <div v-if="!nextProcess">
        ...
        <q-card style="border-radius: 20px 20px" class="q-mx-lg">
          <div>
            <div>
              <q-input class="q-px-md text-SUBTITLE2" v-model="note" />
            </div>
          </div>
        </q-card>
        <div class="q-mx-lg q-mt-lg">
          <q-btn class="full-width" color="primary" label="Next" @click="toNextProcess" />
        </div>
      </div>
...
export default {
  async mounted() {
    ...
    this.accData = JSON.parse(jsonData);
  ...
  data() {
    return {
      ...
      transaction: {
        amount: null,
        timestamp: null,
        from: { acc: null, name: null },
        to: { acc: null, name: null },
      }
    };
  },
  methods: {
    ...
    transactionSlip(routeName){
      const newTransaction = {
        ...
        "timestamp": Date.now(),
        "to": { "acc": Number(this.accData.acc_num), "name": String(this.accData.acc_name) },
