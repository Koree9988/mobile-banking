...
export default {
  async mounted() {
    ...
    const q2 = query(
      collection(this.$db, "account"),
      where("number", "==", Number(newTransaction.to.acc))
    );
    ...
    try {
      ...
      console.log("🚀  this.udata.balance:", this.udata.balance)
      setDoc(upDoc, this.udata)

      const rDoc = doc(this.$db, "account", this.rid);
      this.rdata.balance=Number(this.rdata.balance)+Number(newTransaction.amount)
      console.log("🚀  this.rdata.balance:", this.rdata.balance)
      setDoc(rDoc, this.rdata)
